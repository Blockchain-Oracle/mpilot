...
Per architecture.md ADR-013 (amended) + ADR-015 (new), the visual implementation layer ships as **`@concierge/react-ui`** — NOT scattered across `apps/web/components/`. Designer owns the visual layer of that package; coding agents consume the package; the web app at `concierge.xyz/app` DOGFOODS the package per the web-app-dogfood requirement.

- **Headless behavior + ARIA + state machines** → `@concierge/react` (story-310)
- **Styled drop-in cards** (Radix + shadcn + `@concierge/ui` tokens) → `@concierge/react-ui` (story-311)
- **Designer no longer picks Tambo / Crayon** — both dropped (model-driven, contradict per-tool card contract)
- **Component intent in `08-ux-component-intent.md` is preserved as-is** — that's still the BRIEF. The implementation venue is now `@concierge/react-ui`, not the web app.
...
**Anchor:** **Designer's choice.** Concierge's visual direction is not pinned to a specific competitor or reference site. The designer agent picks the anchor (likely from the Linear / Stripe / Vercel / Granola / v0.dev quality band, but the specific anchor is theirs to choose) and publishes the resulting design tokens + component library back to `@concierge/ui`.
...
Owned by the designer agent. Coding agent does NOT make color/font/motion decisions — it imports from `@concierge/ui/tokens`.
...
**Lock contract:** Once the designer publishes `@concierge/ui@1.0.0`, the tokens are LOCKED. Coding agents read them via import, never override inline.
...
4. Judge runs `npx skills add @concierge/mantle-agent` in their terminal, then drives Concierge from Claude Code via `mcp.concierge.xyz/api/sse`
...
- ❌ Hardcoded hex colors in components — import from `@concierge/ui/tokens`
- ❌ Inline `style={{ ... }}` for non-dynamic values — use Tailwind utilities or token-driven CSS variables
- ❌ Custom font loading via raw `<link>` — fonts ship through `@concierge/ui/fonts` per designer
...
Once `@concierge/ui@1.0.0` is published, coding agents pick up UI stories (Epic 5 onwards).
