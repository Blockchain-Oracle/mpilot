...
|---|---|---|---|
| MCP server is hosted-only Cloudflare Worker | **MCP stdio-first** (`npx -y @concierge/mcp`); hosted Worker is optional secondary | ADR-011 (amended), AUDIT §3 | story-130 (amended), story-133 (amended), story-136 (NEW), story-137 (NEW), story-138 (NEW) |
| Components live in `apps/web/components/`; designer picks framework | Components ship as **`@concierge/react`** (headless) + **`@concierge/react-ui`** (styled) npm packages; tool-ui adopted as DESIGN reference only | ADR-013 (amended), ADR-015 (NEW), SPEC-REWORK-BRIEF Thread 5 | story-310, 311, 312, 313, 314 (all NEW); ux-spec.md amended |
...
| Tools defined per-runtime in each adapter | Single **`@concierge/tools`** framework-agnostic registry with `inputSchema` + `outputSchema` (load-bearing). Adapters (Vercel AI / OpenAI / LangChain / AgentKit / MCP) are 15-40 LOC each, wrap the same source | ADR-014 (NEW), CDR-Kit pattern, SDK-DX-STUDY §H | story-300, 301, 302, 303, 304 (all NEW) |
...
| `@concierge/goat` + `@coinbase/agentkit-vercel-ai-sdk` dependencies | **DROPPED** — GOAT SDK 4-15 months stale; AgentKit framework extensions 15 months stale (use `customActionProvider` escape hatch) | AUDIT §5, §6 | n/a (removed from package list) |
...
| E2 | Shared SDK Core — `@concierge/sdk` skeleton, `@concierge/shared` | 5 | ~6h | E0 |
| E3 | Action Providers — 7 `@concierge/<provider>` packages | 14 | ~22h | E1, E2 |
...
| E9 | Agent Skill Packaging — `@concierge/skill`, SKILL.md, `npx skills add` install | 5 | ~6h | E2, E8 |
...
| **E13** | **Composable Primitive — `@concierge/tools` registry + 4 framework adapters + model-agnostic provider** | **6** | **~7h** | **E2, E3** |
| **E14** | **Composable UI — `@concierge/react` (headless) + `@concierge/react-ui` (styled) + 2 adapters + web dogfood** | **5** | **~10h** | **E13, E7 (scaffold)** |
...
**Business value:** All packages depend on `@concierge/shared` (addresses + ABIs + types) and `@concierge/sdk` skeleton. Without this, providers can't share an interface.
...
**Dependencies:** E4 (smart account), E5 (tick loop), E6 (attestation), **designer agent's `@concierge/ui@1.0.0` published**.

**Estimate:** ~22h.

**Stories:**
- `story-100-next-app-scaffold` — Next.js 15 App Router skeleton, Tailwind, `@concierge/ui` import, routing
...
- `story-153-skill-npm-publish` — Publish `@concierge/mantle-agent` to npm; verify `npx skills add @concierge/mantle-agent` installs cleanly
...
**Business value:** Other Mantle developers `npm install @concierge/sdk` and ship their own agent. Docs convert builders into users. Critical for the "AgentKit for Mantle" positioning + Mantle Ecosystem Contribution score.
...
- `story-172-docs-sdk-reference` — `@concierge/sdk` API reference (auto-generated from JSDoc + manual)
...
**Stories:**
- `story-190-sepolia-mock-deploy` — Deploy `MockAavePool` + mocks + `ConciergeRegistry` to Mantle Sepolia, write addresses to `@concierge/shared`
...
The orchestrator dispatches stories in this order, respecting dependencies. **Designer agent runs in parallel from the start; coding-agent UI stories (E7) wait for `@concierge/ui@1.0.0` to publish.**
...
dispatch_queue:
  ...
  # Wave 6: Web App (BLOCKED until @concierge/ui@1.0.0 ships from designer)
...
Designer agent runs in parallel from the start. UI stories (E7) gate on `@concierge/ui@1.0.0`, but every non-UI surface (contracts, SDK, providers, runtime, MCP, skill, docs) is unblocked from Wave 2 onwards.

---

## Epic E13 — Composable Primitive (NEW 2026-06-09)

**Business value:** Concierge becomes a **composable primitive** — the single `@concierge/tools` registry feeds 4 framework adapters (Vercel AI SDK / OpenAI/Anthropic raw / LangChain / Coinbase AgentKit) + the MCP server. Any developer can `pnpm add @concierge/<adapter>` and drop our 30+ DeFi actions into their existing agent stack in 5 lines. This is the LARGEST single-cost-multiplier win: ~30-40 LOC per adapter × N runtimes = N distribution channels with marginal cost.

**Dependencies:** E2 (`@concierge/sdk` skeleton — amended), E3 (action providers expose `tools()` functions).

**Estimate:** ~7h.

**Stories:**
- `story-300-tools-registry` — `@concierge/tools` framework-agnostic registry. `ConciergeTool<TIn, TOut>` interface with `inputSchema` + `outputSchema` (Zod, both mandatory) + `uiCardId`. `tool()` helper. `createConciergeTools(agent)` aggregator. Per-tool `SerializableConciergeXxxSchema` exports. (Reference: CDR-Kit's `@cdr-kit/tools` shape, code-verified per SDK-DX-STUDY §H.)
- `story-301-vercel-ai-adapter` — `@concierge/vercel-ai` → AI SDK v6 `ToolSet` (~15 LOC wrapper around `aiTool()`)
- `story-302-langchain-adapter` — `@concierge/langchain` → `@langchain/core/tools` `StructuredToolInterface[]` (~10 LOC)
- `story-303-openai-adapter` — `@concierge/openai` → direct Chat Completions `tools: [{ type: 'function', function: { name, description, parameters } }]` + dispatch. Covers Anthropic raw tool-use too (same JSON Schema). NO `@openai/agents` dep.
- `story-304-agentkit-adapter` — `@concierge/agentkit` → Coinbase AgentKit `customActionProvider` (escape hatch — NOT `@CreateAction` decorator) (~12 LOC)
- `story-320-model-agnostic-provider` — `@concierge/sdk` accepts `model: LanguageModelV2` directly. `defaultModel()` helper auto-detects `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GOOGLE_GENERATIVE_AI_API_KEY` / `XAI_API_KEY` with `AI_MODEL="provider:model"` override. Per-call model override per tick phase.

**Acceptance criteria for the epic as a whole:** `pnpm add @concierge/langchain` in an external LangChain app + `getLangChainTools(agent)` returns ≥30 working Concierge tools. Same for Vercel AI SDK + OpenAI + AgentKit. End-to-end demo: a third-party LangChain agent supplies a USDC via Concierge tools, signed by the user's session key.

---

## Epic E14 — Composable UI (NEW 2026-06-09)

**Business value:** Concierge cards (proposal, tick, portfolio, reputation) ship as TWO npm packages — `@concierge/react` (headless: behavior + ARIA + state machines + parse-then-render) and `@concierge/react-ui` (styled drop-ins built on Radix + shadcn + brand tokens). The web app at `concierge.xyz/app` DOGFOODS the package (per ADR-015) — no duplicate components. External devs can `pnpm add @concierge/react-ui` and embed Concierge cards in THEIR dashboard / chat. Two thin adapter packages (`@concierge/react-assistant-ui`, `@concierge/react-copilotkit`) bridge to assistant-ui and CopilotKit — which transitively covers LangGraph / CrewAI / Mastra / Pydantic AI / AutoGen2 / MS Agent Framework users.

**Dependencies:** E13 (`@concierge/tools` for schemas), E7 (Next.js + Tailwind scaffold).

**Estimate:** ~10h.

**Stories:**
- `story-310-react-headless` — `@concierge/react`: `<ProposalPart>`, `<TickPart>`, `<PortfolioPart>`, `<ReputationPart>` taking typed `tool-${name}` parts as props + hooks (`useTickStream`, `useProposal`, `useReputation`) + `<ConciergeProvider>`. ARIA, keyboard nav, state machines. Zero CSS. Render-prop API for consumer's custom visual layer.
- `story-311-react-ui-styled` — `@concierge/react-ui`: `<TickCard>`, `<ProposalCard>`, `<PortfolioCard>`, `<ReputationChart>`, `<EmergencyStop>`, `<GoalInput>`, `<MCPInstallSnippet>`, `<SimulationCard>`, `<TxConfirmationCard>`, `<AttestationCard>`, `<StatusPill>`. Tool-UI patterns as DESIGN REFERENCE only (not dep). 12-state TickCard lifecycle per `08-ux-component-intent.md`. Storybook with ≥12 stories. Axe-core a11y check.
- `story-312-web-dogfood-react-ui` — Rewrite `apps/web/app/app/*` pages to consume `@concierge/react-ui` directly. Delete `apps/web/components/{TickCard,ProposalCard,PortfolioCard,...}.tsx`. NET LOC decreases in apps/web/.
- `story-313-react-assistant-ui` — `@concierge/react-assistant-ui`: `getConciergeToolkit()` returns assistant-ui `defineToolkit({ proposeAction: { type: 'backend', render: ProposalPart }, ... })`. Covers assistant-ui + LangGraph + LangChain users transitively. ~15 LOC.
- `story-314-react-copilotkit` — `@concierge/react-copilotkit`: `useConciergeActions()` hook calling `useCopilotAction` (or `useFrontendTool` v2, pinned at impl time). Covers AG-UI Protocol users transitively: LangGraph + CrewAI + Mastra + Pydantic AI + AutoGen2 + MS Agent Framework. ~30 LOC.

**Acceptance criteria for the epic as a whole:** `pnpm add @concierge/react-ui` in an external Next.js app + render `<TickStream agentId="agt_..." />` shows live ticks. `apps/web/components/` is empty (everything moved to packages). axe-core: 0 critical violations across all cards. Storybook builds with 12+ stories.
...
**Stories:**
- `story-330-scaffolder` — `packages/create-concierge-app/` with `@clack/prompts` CLI + 5 templates. Each template ships `package.json` w/ pinned `@concierge/*` deps + `.env.example` + working `pnpm dev` from clone-to-running in < 5 minutes. Pattern reference: CDR-Kit's `create-cdr-kit-app` with 9 templates.
