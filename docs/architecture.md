...
| UI component library | `@concierge/react-ui` (we ship this) | own | Two-package split per ADR-015: `@concierge/react` (headless) + `@concierge/react-ui` (styled). Schema-driven cards per tool (pattern reference: `@assistant-ui/tool-ui`). Tool-ui adopted as DESIGN reference only — not a runtime dependency. |
...
| LLM SDK (direct) | `@anthropic-ai/sdk` | 0.102.x | Used inside `@concierge/sdk` only for non-Vercel-AI-SDK paths (e.g., MCP server tool internals). Not exposed to consumers. |
...
| MCP transport (default) | **stdio** via `@concierge/mcp` npm package | — | `npx -y @concierge/mcp` is the README default install per ADR-011 (amended). Matches pokaldot/kwala/cdr-kit pattern. |
...
│   ├── shared/                       # @concierge/shared — addresses + ABIs + types
│   │   └── src/{addresses.ts,abi/,types.ts}
│   ├── agent/                        # @concierge/agent — ConciergeAgent class (wallet + RPC + provider singletons)
│   │   └── src/
│   ├── tools/                        # @concierge/tools — framework-agnostic ConciergeTool[] registry (ADR-014)
...
│   # === Framework adapters (wrap @concierge/tools for each runtime, ~15-40 LOC each) ===
│   ├── vercel-ai/                    # @concierge/vercel-ai → ai SDK ToolSet
│   ├── openai/                       # @concierge/openai → OpenAI Chat Completions + Anthropic raw tool-use (one adapter, two runtimes)
│   ├── langchain/                    # @concierge/langchain → @langchain/core/tools StructuredToolInterface[]
│   ├── agentkit/                     # @concierge/agentkit → Coinbase customActionProvider (NOT @CreateAction)
│   # NOTE: @concierge/goat dropped per AUDIT-2026-06-09 §6 (GOAT SDK 4-15 months stale; defer to v1.1)
│   # === MCP server (transport-agnostic core; stdio default + Worker wrapper) ===
│   ├── mcp/                          # @concierge/mcp — transport-agnostic factory + stdio bin (ADR-011 amended)
│   │   └── src/{server.ts,stdio.ts,ui-resources/*.html}
│   # === React components (two-package split per ADR-015) ===
│   ├── react/                        # @concierge/react — headless tool-part components, schema-driven, parse-then-render
│   │   └── src/{ProposalPart.tsx,TickPart.tsx,PortfolioPart.tsx,ReputationPart.tsx,hooks/*.ts}
│   ├── react-ui/                     # @concierge/react-ui — styled drop-ins (Radix + shadcn + Tailwind tokens)
...
│   ├── react-assistant-ui/           # @concierge/react-assistant-ui — assistant-ui defineToolkit adapter (optional)
│   ├── react-copilotkit/             # @concierge/react-copilotkit — useConciergeActions hook (covers AG-UI/LangGraph/CrewAI/Mastra/Pydantic AI) (optional)
│   # === Brand + skill ===
│   ├── ui/                           # @concierge/ui — brand tokens only (color, type, motion, spacing) — designer fills
│   ├── skill/                        # @concierge/skill — Agent Skill (Track 6 qualifier per ADR-003)
│   │   ├── SKILL.md
│   │   ├── package.json              # discoverable by `npx skills add Blockchain-Oracle/concierge`
│   │   └── src/
│   # === SDK meta (convenience re-export) ===
│   └── sdk/                          # @concierge/sdk — convenience meta-package re-exporting agent + tools + vercel-ai
│       └── src/
...
- ❌ **Omitting `outputSchema`** on a `ConciergeTool` — load-bearing for MCP `structuredContent` + Vercel AI SDK `InferUITools` + `@concierge/react-ui` parse-then-render
- ❌ **Subpath-exporting framework adapters from `@concierge/sdk`** — keep adapters as separate packages so peer-dep matrices don't explode
...
There is NO direct Chainlink sUSDe/USD feed on Mantle. Aave Oracle (`0x47a063CfDa980532267970d478EC340C0F80E8df`) routes to Capped composites (sUSDe/USDT/USD + USDC/USD). Concierge reads prices via `IAaveOracle.getAssetPrice(asset)` so health-factor reads align with Aave's liquidation triggers. USDC peg hardcoded at $1 in `@concierge/aave-v3-mantle` price-read helper as defense-in-depth (matches Aave's own treatment).
...
**Amendment:** Concierge MCP ships as **`@concierge/mcp`** — a transport-agnostic core with a stdio binary as the default install path (`claude mcp add concierge -- npx -y @concierge/mcp`). A Cloudflare Worker wrapper at `apps/mcp/` exposes the same core via Streamable HTTP for users who want a URL-paste install. Both consume the same `@concierge/tools` registry — only transport + auth differ.
...
**Files:** `packages/mcp/src/{server.ts (factory), stdio.ts (bin)}` + `apps/mcp/src/index.ts` (Worker wrapper). All tools sourced from `@concierge/tools` per ADR-014.
...
`docs/ux-spec.md` + `research/concierge/08-ux-component-intent.md` describe every component's purpose, states, transitions, streaming behavior, accessibility contract, mobile responsiveness — no framework lock-ins. **2026-06-09 amendment:** components ship in `@concierge/react-ui` (Radix + shadcn + Tailwind tokens) per ADR-015. Designer owns the visual layer of `@concierge/react-ui`; Tambo / Crayon dropped (model-driven, contradict per-tool card contract). The agent runtime is invariant; the published component package is the canonical visual implementation.

### ADR-014 — `@concierge/tools` framework-agnostic tool registry (NEW 2026-06-09)

**Decision:** Concierge action definitions live in ONE source-of-truth package, `@concierge/tools`, with the shape:
...
Plus per-tool `SerializableConciergeXxxSchema` exports (one per `uiCardId`) for parse-then-render in `@concierge/react-ui`.

**Adapters (each ~15-40 LOC) wrap this single registry for one runtime:**
- `@concierge/vercel-ai` → `Object.fromEntries(tools.map(t => [t.name, aiTool({ description, inputSchema, execute })]))`
- `@concierge/openai` → `tools.map(t => ({ type: 'function', function: { name, description, parameters: toJsonSchema(t) } }))` + dispatch (covers Anthropic raw tool-use, same JSON Schema)
- `@concierge/langchain` → `tools.map(t => lcTool(async args => JSON.stringify(await t.invoke(args)), { name, description, schema }))`
- `@concierge/agentkit` → `customActionProvider(tools.map(t => ({ name, description, schema, invoke })))` — **factory escape hatch, NOT `@CreateAction` decorator**
- `@concierge/mcp` → `server.registerTool(name, { description, inputSchema, outputSchema }, async args => ({ content: [...], structuredContent: await t.invoke(args) }))`
...
**Dropped:** `@concierge/goat` (GOAT SDK adapters 15 months stale per AUDIT-2026-06-09 §6); `@concierge/openai-agents` (`@openai/agents` 15 months stale — use Chat Completions direct).

### ADR-015 — Component packaging: `@concierge/react` headless + `@concierge/react-ui` styled (NEW 2026-06-09)

**Decision:** Components ship as TWO npm packages:

- **`@concierge/react`** — headless tool-part components. Each takes a typed `tool-${name}` UI message part as a prop. Renders behavior, ARIA, keyboard nav, state machines. Parse-then-render gated by `safeParseSerializableXxx` from `@concierge/tools`. Zero CSS. Depends on `react` (peer) + `zod` (peer) + `@concierge/tools` (runtime).

- **`@concierge/react-ui`** — styled drop-ins. Re-exports `@concierge/react` headless components wrapped in Radix + shadcn primitives + Tailwind tokens from `@concierge/ui`. Cards: `<TickCard>`, `<ProposalCard>`, `<PortfolioCard>`, `<ReputationChart>`, `<EmergencyStop>`, `<GoalInput>`, `<MCPInstallSnippet>`.

**Distribution: Path C (selected 2026-06-09).** Primary = npm (`pnpm add @concierge/react-ui`). v1.1 stretch = complementary shadcn registry at `concierge.xyz/r/*.json` for copy-paste consumers. **`@assistant-ui/tool-ui` is adopted as DESIGN reference only** — pattern (schema-driven serializable schemas, lifecycle states, mobile-first, parse-then-render), not dependency. Components compose shadcn primitives directly (same building blocks tool-ui uses), MIT.

**Two optional adapter packages** for runtime fan-out:
- **`@concierge/react-assistant-ui`** — wraps `@concierge/react` cards as assistant-ui `defineToolkit({ name: { type: 'backend', render } })`. Covers assistant-ui + LangGraph / LangChain users transitively via the lib's own runtime adapters. ~20 LOC.
- **`@concierge/react-copilotkit`** — exposes `useConciergeActions()` registering each tool via `useCopilotAction({ name, render })`. Covers AG-UI Protocol users transitively: LangGraph + CrewAI + Mastra + Pydantic AI + AutoGen2 + Microsoft Agent Framework. ~25 LOC.
...
**Web-app dogfood requirement:** `apps/web/app/app/*` MUST consume `@concierge/react-ui` directly. No duplication. If a card doesn't fit, fix `@concierge/react-ui`, don't fork.

### ADR-016 — Model-agnostic SDK via `LanguageModelV2` + env auto-detect + per-call override (NEW 2026-06-09)

**Decision:** `@concierge/sdk` is model-agnostic. The public surface accepts `model: LanguageModelV2` directly — users bring their provider via `@ai-sdk/openai` / `@ai-sdk/anthropic` / `@ai-sdk/google` / `@ai-sdk/xai`. No provider wrapping.
...
- Components from `@concierge/react-ui` (per ADR-015)
...
**The contract:** every Concierge tool ships with (a) `inputSchema` (Zod), (b) `outputSchema` (Zod) feeding MCP `structuredContent`, (c) a Vercel AI SDK tool-part card in `@concierge/react-ui` (via `uiCardId`), (d) optionally a `ui://` HTML resource for Rail 2.
...
- `react` (peer `^18 || ^19`) — `@concierge/react` + `@concierge/react-ui` + the two react-adapter packages.
- Framework SDK peer per adapter: `ai ^6` (in `@concierge/vercel-ai`), `@langchain/core ^1` (in `@concierge/langchain`), `@coinbase/agentkit ^1` (in `@concierge/agentkit`), `@modelcontextprotocol/sdk ^1.29` (in `@concierge/mcp`), `@assistant-ui/react ^0.14` (in `@concierge/react-assistant-ui`), `@copilotkit/react-core ^1.59` (in `@concierge/react-copilotkit`).
- **Runtime deps only on internal `@concierge/*` packages** (and `@ai-sdk/provider` for `@concierge/sdk`).
...
pnpm add @concierge/vercel-ai @concierge/sdk   # Vercel AI SDK consumers
pnpm add @concierge/langchain @concierge/sdk   # LangChain consumers
pnpm add @concierge/openai @concierge/sdk      # OpenAI / Anthropic raw tool-use consumers
pnpm add @concierge/agentkit @concierge/sdk    # Coinbase AgentKit consumers
pnpm add @concierge/react-ui                   # Just want our React cards
pnpm add @concierge/react-assistant-ui         # assistant-ui consumers
pnpm add @concierge/react-copilotkit           # CopilotKit / AG-UI consumers (LangGraph, CrewAI, Mastra, Pydantic AI)
```
...
- [ ] **Stdio (default):** `claude mcp add concierge -- npx -y @concierge/mcp` works from Claude Code / Desktop / Cursor / Windsurf / Goose
...
- [ ] **Foundation (3):** `@concierge/shared` + `@concierge/agent` + `@concierge/tools` published with full types
- [ ] **Providers (7):** `@concierge/{aave-v3-mantle, mantle-dex, ethena-susde, ondo-usdy, meth-staking, lifi-bridge, erc8004}` published
- [ ] **Framework adapters (4):** `@concierge/vercel-ai` + `@concierge/openai` + `@concierge/langchain` + `@concierge/agentkit` published
- [ ] **MCP (1):** `@concierge/mcp` published with stdio bin entry; `npx -y @concierge/mcp` runs cleanly
- [ ] **Components (4):** `@concierge/react` + `@concierge/react-ui` + `@concierge/react-assistant-ui` + `@concierge/react-copilotkit` published
- [ ] **Meta (1):** `@concierge/sdk` convenience re-export published
...
- [ ] `pnpm add @concierge/sdk` from a fresh project + the 5-line `defaultModel()` quickstart works end-to-end
- [ ] At least one adapter verified working: `pnpm add @concierge/langchain` in an external LangChain app calls a Concierge tool successfully
