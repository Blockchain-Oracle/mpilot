...
Note on package scope: rationale columns reference the new `@concierge-mantle/*` scope (per the in-flight rebrand from `@concierge/*`), since the reconciliation will land after the rebrand branch merges.
