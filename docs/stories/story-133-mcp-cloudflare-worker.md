...
2. **`apps/mcp/src/index.ts` is THIN** — imports `createStreamableHttpHandler` from `@concierge/mcp` and wraps it in a Hono app with Workers-specific bindings (KV / D1 / secrets / custom domain).
...
- `apps/mcp/src/index.ts` — NEW — Hono app, imports `createStreamableHttpHandler` from `@concierge/mcp`, wires bearer-token middleware reading from `c.env.CONCIERGE_BEARER_TOKEN_HASH_MAP` (KV) and `c.env.MAINNET_RPC_URL` (secret)
...
Then response includes all @concierge/tools tools AND the response context references agt_xyz

Given the same code path runs locally and on Workers
When `pnpm --filter @concierge/mcp test` runs the StreamableHTTP transport in a unit test
Then the same handler works (the factory is environment-agnostic)
```

### Updated notes

- **DO NOT re-implement tool registration here.** Import `createStreamableHttpHandler` from `@concierge/mcp` (built in story-130 amended).
