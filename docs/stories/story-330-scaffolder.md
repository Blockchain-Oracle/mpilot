...
- `packages/create-concierge-app/templates/starter/` — NEW — minimal Node + `@mpilot/sdk` 5-line tick loop
- `packages/create-concierge-app/templates/vercel-ai-agent/` — NEW — Next.js + Vercel AI SDK + `@mpilot/vercel-ai` + `@mpilot/react-ui` (the canonical chat app)
- `packages/create-concierge-app/templates/langchain-agent/` — NEW — `@mpilot/langchain` + LangChain agent executor
- `packages/create-concierge-app/templates/mcp-only/` — NEW — Cloudflare Worker wrapping `packages/mcp` for hosted MCP
- `packages/create-concierge-app/templates/react-embed/` — NEW — Drop-in React component example (consumes `@mpilot/react-ui`)
...
# Each template's package.json should reference concrete @mpilot/* deps (proves cross-package linkage)
for t in starter vercel-ai-agent langchain-agent react-embed; do
  grep -q '"@mpilot/' packages/create-concierge-app/templates/${t}/package.json || { echo "template $t lacks @mpilot/* dep"; exit 1; }
done
