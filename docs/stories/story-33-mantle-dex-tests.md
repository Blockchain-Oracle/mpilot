# Story — `@mpilot/mantle-dex` integration tests
...
When `pnpm --filter @mpilot/mantle-dex run test` runs
...
Given coverage gate
When `pnpm --filter @mpilot/mantle-dex run test --coverage` runs
...
# Tests pass
pnpm --filter @mpilot/mantle-dex run test --reporter=verbose
test $? -eq 0

# ≥ 25 test cases
pnpm --filter @mpilot/mantle-dex run test --reporter=verbose 2>&1 | grep -E "(✓|PASS)" | wc -l | awk '$1 >= 25 {exit 0} {exit 1}'
...
for tn in SlippageBreach_RevertsBeforeSubmit RequoteOnExecute_PicksFreshBest NoRouteVenueReturnsNullCleanly; do
  pnpm --filter @mpilot/mantle-dex run test --reporter=verbose 2>&1 | grep "$tn" | grep -q "✓" || { echo "missing $tn"; exit 1; }
done

# Coverage ≥ 80%
cov=$(pnpm --filter @mpilot/mantle-dex run test --coverage 2>&1 | grep "All files" | awk '{print $4}' | tr -d '%')
test "${cov%.*}" -ge 80
