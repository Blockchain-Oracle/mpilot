# Story — `@concierge/erc8004` integration tests
...
When `pnpm --filter @concierge/erc8004 run test` runs
...
Given coverage
When `pnpm --filter @concierge/erc8004 run test --coverage` runs
...
```bash
pnpm --filter @concierge/erc8004 run test --reporter=verbose
test $? -eq 0

pnpm --filter @concierge/erc8004 run test --reporter=verbose 2>&1 | grep -E "(✓|PASS)" | wc -l | awk '$1 >= 22 {exit 0} {exit 1}'
...
for tn in "HashDeterminism" "schemaIdFor_Deterministic" "AgainstNonExistentAgent" "ResolvesSepolia" "ResolvesMainnet"; do
  pnpm --filter @concierge/erc8004 run test --reporter=verbose 2>&1 | grep "$tn" | grep -q "✓" || { echo "missing $tn"; exit 1; }
done

# Coverage ≥ 90%
cov=$(pnpm --filter @concierge/erc8004 run test --coverage 2>&1 | grep "All files" | awk '{print $4}' | tr -d '%')
test "${cov%.*}" -ge 90
