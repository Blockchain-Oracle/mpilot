...
**As a** Claude Code user who just ran `npx skills add @concierge/mantle-agent`
...
# Tests pass
pnpm --filter @concierge/skill-mantle-agent run test 2>&1 | grep "references" | grep -q "PASS"
...
- **Troubleshooting walks through real failures.** Each section: symptom → likely cause → fix. Don't generalize ("if it doesn't work, check your config") — be specific ("if you see 401 errors, your OAuth token expired; run `npx skills config @concierge/mantle-agent --refresh`").
