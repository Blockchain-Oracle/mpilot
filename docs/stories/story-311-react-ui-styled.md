# Story — `@concierge/react-ui` styled drop-in cards
...
**I want to** `pnpm add @concierge/react-ui` and drop `<TickCard />`, `<ProposalCard />`, `<PortfolioCard />` etc. into my app, styled with Concierge brand tokens
...
- `packages/react-ui/package.json` — NEW — ESM-only per ADR-018; peer deps on `react`, `react-dom`, `@concierge/react`, `tailwindcss ^4`; runtime deps on `@concierge/ui` (brand tokens), Radix primitives, `class-variance-authority`, `lucide-react`
- `packages/react-ui/src/TickCard.tsx` — NEW — composes `<TickPart>` from `@concierge/react`; renders the 12 lifecycle states from `08-ux-component-intent.md` § TickCard
...
When `pnpm --filter @concierge/react-ui storybook:build` runs
...
# Peer deps shape — @concierge/react MUST be peer (not runtime)
node -e "
  const p = require('./packages/react-ui/package.json');
  if (p.dependencies?.['@concierge/react']) process.exit(1);
  if (!p.peerDependencies?.['@concierge/react']) process.exit(2);
"
...
pnpm --filter @concierge/react-ui test:a11y 2>&1 | grep -q "0 violations"

pnpm --filter @concierge/react-ui build
```
...
- **Per-card schema gating** (per ADR-014): every card calls `safeParseSerializableXxx(part.output)` (from `@concierge/react` which inherits from `@concierge/tools`) before rendering content. On parse fail → error region.
...
- **Brand tokens** come from `@concierge/ui` (the brand-token-only package). Designer agent populates that; this story just CONSUMES the tokens.
