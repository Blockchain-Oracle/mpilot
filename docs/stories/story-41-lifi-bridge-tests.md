# Story — `@concierge/lifi-bridge` integration tests
...
When `pnpm --filter @concierge/lifi-bridge run test` runs
...
Given coverage
When `pnpm --filter @concierge/lifi-bridge run test --coverage` runs
...
```bash
pnpm --filter @concierge/lifi-bridge run test --reporter=verbose
test $? -eq 0

pnpm --filter @concierge/lifi-bridge run test --reporter=verbose 2>&1 | grep -E "(✓|PASS)" | wc -l | awk '$1 >= 18 {exit 0} {exit 1}'
...
for tn in "StaleRouteRefresh" "two_stage_attestation_Linkage" "sessionKey_PolicyRestrictive" "FilteredBridges"; do
  pnpm --filter @concierge/lifi-bridge run test --reporter=verbose 2>&1 | grep "$tn" | grep -q "✓" || { echo "missing $tn"; exit 1; }
done

# Coverage ≥ 85%
cov=$(pnpm --filter @concierge/lifi-bridge run test --coverage 2>&1 | grep "All files" | awk '{print $4}' | tr -d '%')
test "${cov%.*}" -ge 85
