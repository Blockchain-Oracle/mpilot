...
- `apps/web/components/landing/DeveloperCTA.tsx` — NEW — block with headline "Built for developers" or similar; code blocks showing `pnpm add @concierge/sdk` and a 5-line example of using the SDK to register an agent + read reputation. Links to /docs.
...
When inspected
Then it contains a code block with `pnpm add @concierge/sdk` AND a 5-line example AND a link to /docs
...
grep -q "pnpm add @concierge/sdk" apps/web/components/landing/DeveloperCTA.tsx

# /docs link present
grep -q "/docs" apps/web/components/landing/DeveloperCTA.tsx

# Tests pass
pnpm --filter @concierge/web run test 2>&1 | grep "DeveloperCTA" | grep -q "PASS"
...
- **The 5-line example** should do something concrete: register an agent, read its reputation. NOT "import { Concierge } from '@concierge/sdk'" alone — too abstract. Reference: `research/concierge/03-providers/erc8004.md` § Integration pattern for a real-shape example.
