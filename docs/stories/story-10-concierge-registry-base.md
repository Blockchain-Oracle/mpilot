...
- **UUPS-upgradeable** because we may need to fix bugs post-deploy in hackathon window without a full redeploy of the proxy (which would break `@concierge/shared/addresses.ts`). The proxy uses OZ's `ERC1967Proxy` + `UUPSUpgradeable`. `_authorizeUpgrade` is gated on `ADMIN_ROLE`.
