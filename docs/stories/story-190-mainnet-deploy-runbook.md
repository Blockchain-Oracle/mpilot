...
**I want to** a step-by-step runbook covers: pre-deploy checks (CI green, audit findings closed, gas funding), the exact `forge script` command, post-deploy verification (Mantlescan source verification, on-chain `cast call` round-trip), addresses publication to `@concierge/shared`, rollback procedure
