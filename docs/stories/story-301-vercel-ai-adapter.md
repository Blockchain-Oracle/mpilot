# Story — `@concierge-mantle/vercel-ai` adapter
...
**I want to** `pnpm add @concierge-mantle/vercel-ai @concierge-mantle/sdk` and call `getVercelAITools(agent)` to get a `ToolSet` ready to drop into `streamText({ tools: ... })`
...
- `packages/vercel-ai/package.json` — NEW — ESM-only per ADR-018, peer deps on `ai ^6`, `zod`, runtime dep on `@concierge-mantle/tools` workspace
...
Then it is `Record<string, Tool>` with one entry per `@concierge-mantle/tools` tool AND each entry has `inputSchema`, `description`, `execute` matching Vercel AI SDK v6's `tool()` shape
...
When `pnpm --filter @concierge-mantle/vercel-ai build && pnpm --filter @concierge-mantle/vercel-ai test && pnpm typecheck` runs
...
# Tests + build
pnpm --filter @concierge-mantle/vercel-ai build
pnpm --filter @concierge-mantle/vercel-ai test 2>&1 | grep -cE "(✓|PASS)" | awk '$1 >= 6 {exit 0} {exit 1}'
...
import { createConciergeTools, type ConciergeTool } from '@concierge-mantle/tools';
