# Story — `@concierge/agentkit` adapter (Coinbase AgentKit, `customActionProvider` path)
...
Then the action's `invoke(args)` routes to `ConciergeTool.invoke()` from @concierge/tools
...
When `pnpm --filter @concierge/agentkit build && pnpm --filter @concierge/agentkit test` runs
...
pnpm --filter @concierge/agentkit build
pnpm --filter @concierge/agentkit test 2>&1 | grep -cE "(✓|PASS)" | awk '$1 >= 5 {exit 0} {exit 1}'
```
