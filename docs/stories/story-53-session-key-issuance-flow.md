...
# Tests pass
pnpm --filter @mpilot/smart-account run test 2>&1 | grep "issueSessionKey" | grep -q "PASS"

# Encrypted-at-rest invariant verified
pnpm --filter @mpilot/smart-account run test 2>&1 | grep -E "Encrypted|persist" | grep -q "PASS"
