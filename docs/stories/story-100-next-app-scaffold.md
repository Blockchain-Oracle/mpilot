...
- `apps/web/package.json` — NEW — Next.js 15, React 19, Tailwind v4, peer deps on `@concierge/sdk`, `@concierge/shared`, `@concierge/runtime` (read-only types)
...
Given typecheck
When `pnpm --filter @concierge/web run typecheck` runs
Then exit code is 0
