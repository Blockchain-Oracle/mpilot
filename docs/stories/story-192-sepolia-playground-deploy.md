...
When read
Then it covers: forge deploy command, expected addresses (auto-synced to @concierge/shared), distributor funding ($500 USDC + 100 MNT to start)
...
# Tests pass
pnpm --filter @concierge/web run test 2>&1 | grep "InAppFaucet" | grep -q "PASS"
