# Story — `@concierge/erc8004` action provider
...
**I want to** an `@concierge/erc8004` package exposes `registerAgent`, `attestAction`, `readReputation`, `readFeedback` actions against the ERC-8004 Identity + Reputation registries on Mantle Mainnet (and Sepolia testnet) using canonical addresses + ABIs
...
When `pnpm --filter @concierge/erc8004 run build` runs
Then exit code is 0
...
pnpm --filter @concierge/erc8004 run build
test $? -eq 0
pnpm run typecheck
...
- **ABIs come from `@concierge/shared`** (story-21) which fetched them from the canonical `erc-8004/erc-8004-contracts` repo via `gh api`. NEVER hand-type ABI signatures — they drift silently. Re-verify ABIs against the canonical source before any new attestation action.
