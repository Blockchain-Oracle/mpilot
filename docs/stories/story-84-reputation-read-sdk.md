...
pnpm --filter @concierge/attestation run test 2>&1 | grep "loadAgentHistory" | grep -q "PASS"

# Cache hit avoids IPFS calls
pnpm --filter @concierge/attestation run test --reporter=verbose 2>&1 | grep "cache.*hit" | grep -q "✓"
