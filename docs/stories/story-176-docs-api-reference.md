...
- `apps/web/content/docs/reference/errors.mdx` — NEW — handwritten typed error reference (every error from @mpilot/sdk + how to handle)
...
Then it produces MDX files in apps/web/content/docs/reference/sdk/ for every public export of @mpilot/sdk

Given a public function signature changes in @mpilot/sdk
...
When inspected
Then it lists every error class from @mpilot/sdk/errors with: error code, when it fires, recovery action
