# Story — `@concierge/ethena-susde` action provider
...
**I want to** an `@concierge/ethena-susde` package exposes `getYieldRate`, `getCarryVsAave`, `wrapToSusde`, `unwrapToUSDe` actions (no on-chain native staking — that's Ethereum L1) plus reads for the Mantle-side bridged sUSDe
...
When `pnpm --filter @concierge/ethena-susde run build` runs
Then exit code is 0
...
pnpm --filter @concierge/ethena-susde run build
test $? -eq 0
pnpm run typecheck
test $? -eq 0
...
- **Composes with Aave provider** for `getCarryVsAave` — imports `quoteBorrowAPR` from `@concierge/aave-v3-mantle`. Listed as a peer dep in `package.json`.
