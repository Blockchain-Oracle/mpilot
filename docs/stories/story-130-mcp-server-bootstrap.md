...
3. **`packages/mcp/package.json` `bin` field**:
   ...
   ```
   `npx -y @mpilot/mcp` runs the stdio bin by default.
...
- `packages/mcp/package.json` — NEW — `"type": "module"`, `"sideEffects": false`, `bin: { "concierge-mcp": "./dist/stdio.js" }`, deps on `@mpilot/tools` + `@mpilot/agent` + `@mpilot/shared`, peer dep on `@modelcontextprotocol/sdk ^1.29` + `zod ^3.25 || ^4.1`
- `packages/mcp/src/server.ts` — NEW — `createConciergeMcpServer({ agent }: { agent: ConciergeAgent })` factory; returns `McpServer` with all `@mpilot/tools` registered + `outputSchema` per tool
...
When `pnpm --filter @mpilot/mcp build` runs
...
- **DO use `outputSchema` mandatorily** per tool. The tool comes from `@mpilot/tools` which already requires it — just pass `t.outputSchema.shape` to `registerTool`.
...
- `apps/mcp-server/package.json` — NEW — workspace package; deps: `hono`, `@modelcontextprotocol/sdk` v2, `@mpilot/sdk`, `@mpilot/shared`
...
```
Given the package builds
When `pnpm --filter @mpilot/mcp-server run build` runs
Then exit code is 0

Given the local dev server runs
When `pnpm --filter @mpilot/mcp-server dev` runs
...
Given typecheck
When `pnpm --filter @mpilot/mcp-server run typecheck` runs
Then exit code is 0
...
# Tests pass
pnpm --filter @mpilot/mcp-server run test 2>&1 | grep "server" | grep -q "PASS"
