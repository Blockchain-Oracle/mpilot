# Story — `@mpilot/ondo-usdy` integration tests
...
When `pnpm --filter @mpilot/ondo-usdy run test` runs
...
Given coverage
When `pnpm --filter @mpilot/ondo-usdy run test --coverage` runs
...
```bash
pnpm --filter @mpilot/ondo-usdy run test --reporter=verbose
test $? -eq 0

pnpm --filter @mpilot/ondo-usdy run test --reporter=verbose 2>&1 | grep -E "(✓|PASS)" | wc -l | awk '$1 >= 12 {exit 0} {exit 1}'

# Critical guard: no mutation actions
pnpm --filter @mpilot/ondo-usdy run test --reporter=verbose 2>&1 | grep "NoMutationActions" | grep -q "✓"

# Coverage ≥ 80%
cov=$(pnpm --filter @mpilot/ondo-usdy run test --coverage 2>&1 | grep "All files" | awk '{print $4}' | tr -d '%')
test "${cov%.*}" -ge 80
