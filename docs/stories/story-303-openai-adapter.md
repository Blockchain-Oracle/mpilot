# Story — `@concierge/openai` adapter (covers OpenAI Chat Completions AND Anthropic raw tool-use)
...
When `pnpm --filter @concierge/openai build && pnpm --filter @concierge/openai test` runs
...
pnpm --filter @concierge/openai build
pnpm --filter @concierge/openai test 2>&1 | grep -cE "(✓|PASS)" | awk '$1 >= 6 {exit 0} {exit 1}'
```
...
```typescript
import { createConciergeTools, toJsonSchema } from '@concierge/tools';
