...
- `apps/worker/package.json` — NEW — Node process workspace package, deps on `@concierge/runtime`, `bullmq`, `ioredis`, `pino`, `@sentry/node`
...
- `apps/worker/src/tickJob.ts` — NEW — the BullMQ Worker's job processor. Calls `tick(agentId)` from `@concierge/runtime`, captures result, updates ticks table, handles errors via DLQ routing.
...
When `pnpm --filter @concierge/worker dev` runs locally with Redis connected
...
pnpm --filter @concierge/worker run build
test $? -eq 0
pnpm run typecheck
