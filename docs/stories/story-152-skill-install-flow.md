# Story — Skill install flow (`npx skills add @mpilot/mantle-agent`)
...
```
Given `npx skills add @mpilot/mantle-agent` is run
...
pnpm --filter @mpilot/skill-mantle-agent run build
test $? -eq 0
...
# Tests pass
pnpm --filter @mpilot/skill-mantle-agent run test 2>&1 | grep "install" | grep -q "PASS"
