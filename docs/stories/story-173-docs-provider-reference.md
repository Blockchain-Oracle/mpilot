...
When inspected
Then they use the AddressTable component pulling from @mpilot/shared/addresses (single source of truth — no hardcoded addresses in MDX)
...
- **No hardcoded addresses in MDX.** Use the AddressTable component that imports from `@mpilot/shared/addresses`. When addresses change (e.g., new Sepolia mock deploy), all docs update automatically.
