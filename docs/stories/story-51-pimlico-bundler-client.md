...
**As a** mPilot smart-account user
...
**So that** UserOps execute reliably on Mantle without spinning up a self-hosted bundler, and judges trying mPilot on Sepolia get gasless onboarding
...
- `packages/smart-account/src/paymaster.ts` — NEW — `createPaymasterClient({ chain, apiKey, sponsorshipPolicy })` configures Pimlico's verifying paymaster. Two modes: `sponsorshipPolicy: 'always'` (Sepolia demo — mPilot pays gas) or `sponsorshipPolicy: 'never'` (Mainnet production — user pays gas via account's native MNT balance).
...
- **`sponsorshipPolicy` config drives the per-chain default.** Sepolia → `'always'` (mPilot sponsors via paymaster; judge demo costs us a few dollars total). Mainnet → `'never'` (user pays via their account's native MNT — they have to fund the smart account before first action).
