...
**As a** mPilot user installing the MCP server in Claude Code
**I want to** the first tool call triggers an OAuth flow (redirect to mpilot.xyz/oauth/mcp-authorize), I sign in via Privy, and a token is bound to my user — subsequent tool calls work without re-prompting, and per-token rate limits prevent abuse
...
- `apps/web/app/oauth/mcp-authorize/page.tsx` — NEW — the user-facing OAuth consent screen on the web app side (NOT in mcp-server — the consent UI lives on mpilot.xyz)
...
```
Given the user runs `claude mcp add concierge https://mcp.mpilot.xyz/mcp`
...
- **`WWW-Authenticate` header on 401** is the standard signal for MCP clients to initiate OAuth. Format: `Bearer realm="concierge", authorization_uri="https://mpilot.xyz/oauth/authorize"`.
