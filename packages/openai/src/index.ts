...
export function toOpenAITool(t: ConciergeTool): OpenAIFunctionTool {
  ...
  if (parameters['type'] !== 'object') {
    throw new TypeError(
      `[@mpilot/openai] toOpenAITool: expected a root type:"object" schema for tool "${t.name}", got ${JSON.stringify(parameters['type'])} — z.toJSONSchema emission may have changed; pin/check the zod version.`,
    );
  }
  return {
    type: 'function',
  ...
  return {
    ...
    async dispatch(name, args) {
      ...
      if (!t) {
        ...
        throw new Error(
          `[@mpilot/openai] dispatch: unknown tool "${name}". ${
            known.length === 0
      ...
      if (typeof args === 'string') {
        try {
          ...
          throw new SyntaxError(
            `[@mpilot/openai] dispatch("${name}"): malformed JSON arguments — ${
      ...
      try {
        ...
        if (isZodError(err)) {
          try {
            err.message = `[@mpilot/openai] dispatch("${name}"): arguments failed inputSchema validation — ${err.message}`;
          } catch {
