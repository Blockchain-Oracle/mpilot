  ...
  for (const log of receipt.logs) {
    ...
    try {
      ...
      throw new ConciergeError(
        'RpcError',
        `[@mpilot/aave-v3-mantle] claimRewards: failed to decode RewardsClaimed event. Pool ABI may have changed.`,
...
async function executeClaimRewards(ctx: ActionContext, args: z.infer<typeof ClaimRewardsInput>) {
  ...
  if (!incentivesControllerAddress) {
    throw new ConciergeError(
      'NetworkUnsupported',
      '[@mpilot/aave-v3-mantle] claimRewards: incentives controller is not deployed on this chain. Use Mantle Mainnet or provide an incentivesController override.',
    );
  }
  ...
  try {
    ...
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/aave-v3-mantle] claimRewards: claimAllRewards() failed on controller ${incentivesControllerAddress}. Verify the assets array contains valid aToken/debtToken addresses.`,
  ...
  if (receipt.status === 'reverted') {
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/aave-v3-mantle] claimRewards: tx ${txHash} was mined but REVERTED. Verify the assets array contains valid aToken/debtToken addresses with accrued rewards.`,
      undefined,
      { txHash },
    );
  }
