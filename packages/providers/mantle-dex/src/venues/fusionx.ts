  ...
  async function swap(params: VenueSwapParams): Promise<VenueSwapResult> {
    if (!walletClient) {
      throw new ConciergeError(
        'ConfigError',
        '[@mpilot/mantle-dex] fusionx.swap: walletClient required',
      );
    }
    ...
    try {
      ...
      if (ao === 0n) {
        throw new ConciergeError(
          ...
          `[@mpilot/mantle-dex] fusionx.swap: quoter returned zero for ${tokenIn} → ${tokenOut}`,
        );
      }
      freshAmountOut = ao;
    } catch (err) {
      ...
      if (err instanceof BaseError && err.walk((e) => e instanceof ContractFunctionRevertedError)) {
        throw new ConciergeError(
          'InsufficientLiquidity',
          `[@mpilot/mantle-dex] fusionx.swap: no route for ${tokenIn} → ${tokenOut}`,
        );
      }
      throw err;
    }
    ...
    if (receipt.status === 'reverted') {
      throw new ConciergeError(
        'RpcError',
        `[@mpilot/mantle-dex] fusionx.swap: tx ${txHash} reverted`,
      );
    }
