...
async function doFetch(url: string, init: RequestInit, label: string): Promise<unknown> {
  ...
  try {
    ...
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/lifi-bridge] ${label}: network error`,
  ...
  if (!res.ok) {
    ...
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/lifi-bridge] ${label}: HTTP ${res.status}${detail}`,
    );
  }
  ...
  try {
    ...
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/lifi-bridge] ${label}: response is not valid JSON`,
      err instanceof Error ? err : undefined,
    );
  }
}
...
export async function fetchQuote(params: GetQuoteParams): Promise<LifiBridgeRoute | null> {
  ...
  if (!parsed.success)
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/lifi-bridge] fetchQuote: unexpected response shape — ${parsed.error.message}`,
    );
  ...
  if (!route)
    throw new ConciergeError(
      'RpcError',
      '[@mpilot/lifi-bridge] fetchQuote: Li.Fi returned a route missing required amount fields',
    );
  return route;
}
...
export async function fetchBridgeStatus(params: GetStatusParams): Promise<LifiStatusResponse> {
  ...
  if (!parsed.success)
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/lifi-bridge] fetchBridgeStatus: unexpected response shape — ${parsed.error.message}`,
    );
  return parsed.data;
}
