...
export function computeRateFromSqrt(sqrtPriceX96: bigint): bigint {
  if (sqrtPriceX96 === 0n) {
    throw new ConciergeError(
      'RpcError',
      '[@mpilot/meth-staking] computeRateFromSqrt: sqrtPriceX96 is zero — pool may be uninitialized',
    );
  }
  const Q192 = 2n ** 192n;
  ...
  if (rate === 0n) {
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/meth-staking] computeRateFromSqrt: sqrtPriceX96 (${sqrtPriceX96}) too small — rate not representable`,
    );
  }
  ...
  if (rate < MIN_RATE || rate > MAX_RATE) {
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/meth-staking] computeRateFromSqrt: computed rate ${rate} outside sanity bounds [0.5e18, 10e18]`,
    );
  }
  return rate;
}
...
function computeYieldBps(
  ...
  if (diff > MAX_SAFE || diff < -MAX_SAFE) {
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/meth-staking] computeYieldBps: tick cumulative diff (${diff}) outside safe integer range`,
    );
  }
...
export async function fetchPoolState(
  ...
  return publicClient
    ...
    .catch((err: unknown) => {
      throw new ConciergeError(
        'RpcError',
        `[@mpilot/meth-staking] ${tag}: failed to read mETH/WETH pool slot0`,
...
export async function fetchYieldBps(
  ...
  try {
    ...
    throw new ConciergeError(
      ...
      isOldRevert
        ? `[@mpilot/meth-staking] ${tag}: mETH/WETH pool has fewer than 7 days of observations — yield calculation unavailable`
        : `[@mpilot/meth-staking] ${tag}: failed to call observe() on mETH/WETH pool`,
      err instanceof Error ? err : undefined,
    );
  }
  ...
  if (t0 === undefined || t1 === undefined) {
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/meth-staking] ${tag}: observe() returned fewer tick cumulatives than expected`,
    );
  }
  ...
  if (yieldBps <= 0) {
    throw new ConciergeError(
      ...
      `[@mpilot/meth-staking] ${tag}: computed mETH yield <= 0 bps (${yieldBps}) — pool may be too new or price feed unreliable`,
    );
  }
  if (yieldBps > 10_000) {
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/meth-staking] ${tag}: computed yield ${yieldBps} bps exceeds sanity ceiling (10,000 bps / 100% APY)`,
    );
  }
  return yieldBps;
}
