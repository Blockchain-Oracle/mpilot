import { type DbClient, type EoaTx, eoaTxQueue } from '@mpilot/db';
...
function assertUuid(value: string, field: string): void {
  if (!uuidSchema.safeParse(value).success) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] queue: ${field} is not a valid UUID.`,
    );
  }
}
...
function assertEnqueueInput(input: EnqueueInput): void {
  if (!userIdSchema.safeParse(input.userId).success) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] enqueue: userId must be 1-256 chars.`,
    );
  }
  ...
  if (!addressSchema.safeParse(input.to).success) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] enqueue: to is not a valid address.`,
    );
  }
  if (!hexSchema.safeParse(input.data).success) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] enqueue: data is not byte-aligned 0x-prefixed hex.`,
    );
  }
  if (!valueSchema.safeParse(input.value).success) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] enqueue: value is not an unsigned-decimal-string wei ≤ 78 digits.`,
    );
  }
}
...
export async function enqueue(
  ...
  if (!row) {
    ...
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] enqueue: INSERT ... RETURNING returned no row (DB invariant violated).`,
    );
  }
  return row;
}
...
export async function getPending(
  ...
  if (!userIdSchema.safeParse(args.expectedUserId).success) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] getPending: expectedUserId must be 1-256 chars.`,
    );
  }
  return db
    .select()
...
export async function markFailed(
  ...
  if (!errorMsgSchema.safeParse(args.error).success) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] markFailed: error must be 1-2048 chars (silent-failure rule + DB CHECK).`,
    );
  }
  const [row] = await db
    .update(eoaTxQueue)
