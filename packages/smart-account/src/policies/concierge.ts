...
function mergePermissions(perms: readonly CallPermission[]): CallPermission[] {
    ...
    if (existing) {
      ...
      if (existingHasRules || newHasRules) {
        ...
        throw new ConciergeError(
          'ConfigError',
          `[@mpilot/smart-account] createConciergePolicy: InvalidPolicy: conflicting permissions on (target=${target}, selector=${sel}) — ${which} carries rules; merging would silently drop a constraint.`,
        );
      }
      continue;
    }
  ...
  for (const t of targetsWithWildcard) {
    if (targetsWithSpecific.has(t)) {
      throw new ConciergeError(
        'ConfigError',
        `[@mpilot/smart-account] createConciergePolicy: InvalidPolicy: target ${t} has both a wildcard permission (no selector) and a specific-selector permission — the wildcard would void the specific restriction. Either remove the wildcard or list specific selectors explicitly.`,
      );
    }
  }
  return [...byKey.values()];
...
export function createConciergePolicy(config: CreateConciergePolicyConfig): readonly Policy[] {
  if (config.providers.length === 0) {
    throw new ConciergeError(
      'ConfigError',
      '[@mpilot/smart-account] createConciergePolicy: InvalidPolicy: at least one provider required — empty policy bundle would issue an unconstrained session key.',
    );
  }
  ...
  for (const limit of config.spendingLimits) {
    ...
    if (seenTokens.has(t)) {
      throw new ConciergeError(
        'ConfigError',
        `[@mpilot/smart-account] createConciergePolicy: InvalidPolicy: duplicate spendingLimits entry for token ${limit.token} — multiple LTE rules on the same (token, transfer) permission are not supported.`,
      );
    }
    seenTokens.add(t);
  }
  ...
  if (merged.length === 0) {
    throw new ConciergeError(
      'ConfigError',
      '[@mpilot/smart-account] createConciergePolicy: InvalidPolicy: no permissions after merge — providers contributed an empty allow-list.',
    );
  }
