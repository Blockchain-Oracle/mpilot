...
export async function issueSessionKey(
  ...
  if (!chainConfig) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] issueSessionKey: UnsupportedChain('${config.chain}') — supported: ${Object.keys(CHAIN_CONFIGS).join(', ')}`,
    );
  }
  ...
  if (validUntil <= validAfter) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] issueSessionKey: validUntil (${validUntil}) must be > validAfter (${validAfter}).`,
    );
  }
  if (validUntil <= now) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/smart-account] issueSessionKey: validUntil (${validUntil}) is already in the past (now=${now}).`,
    );
  }
  ...
  try {
    ...
    if (recovered.toLowerCase() !== config.ownerAccount.address.toLowerCase()) {
      throw new ConciergeError(
        ...
        `[@mpilot/smart-account] issueSessionKey: EIP-712 signature recovery mismatch — recovered '${recovered}' but expected '${config.ownerAccount.address}'. Owner signTypedData callback may be broken or returning garbage.`,
      );
    }
...
async function readValidatorNonce(client: PublicClient, accountAddress: Address): Promise<number> {
  if (typeof getKernelV3Nonce !== 'function' && typeof accountMetadata !== 'function') {
    throw new ConciergeError(
      'ConfigError',
      '[@mpilot/smart-account] issueSessionKey: @zerodev/sdk version drift — neither getKernelV3Nonce nor accountMetadata is callable. Update @zerodev/sdk.',
    );
  }
  try {
    ...
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/smart-account] issueSessionKey: failed to read kernel validator nonce for ${accountAddress}.`,
      err,
    );
  }
}
