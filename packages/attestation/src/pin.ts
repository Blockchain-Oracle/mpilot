...
export async function pinFeedback(
  ...
  if (deps.primary === undefined && deps.fallback === undefined) {
    throw new ConciergeError(
      'ConfigError',
      '[@mpilot/attestation] pinFeedback: at least one of `primary` or `fallback` PinService must be configured.',
    );
  }
  ...
  if (!primary.ok && !fallback.ok) {
    ...
    throw new ConciergeError(
      'IPFSPinFailed',
      `[@mpilot/attestation] pinFeedback: BOTH services failed. primary=${primary.error} | fallback=${fallback.error}`,
      undefined,
