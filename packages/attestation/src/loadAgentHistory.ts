...
/** Raw on-chain feedback entries — matches `@mpilot/erc8004` FeedbackEntrySchema. */
...
export async function loadAgentHistory(
  ...
  if (!Number.isInteger(limit) || limit < 1 || limit > MAX_LIMIT) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/attestation] loadAgentHistory: limit must be 1..${MAX_LIMIT} (got ${limit}).`,
    );
  }
  if (!Number.isInteger(offset) || offset < 0) {
    throw new ConciergeError(
      'ConfigError',
      `[@mpilot/attestation] loadAgentHistory: offset must be ≥ 0 (got ${offset}).`,
    );
  }
  if (inputs.agentId < 0n) {
    throw new ConciergeError(
      'ConfigError',
      '[@mpilot/attestation] loadAgentHistory: agentId must be ≥ 0.',
    );
  }
