 ...
 * @mpilot/agent free of a hard @mpilot/db dependency.
 */
...
export async function runPropose(
  ...
  if (!KIND_SET.has(inputs.kind)) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: unknown kind '${inputs.kind}'.`,
    );
  }
  if (!PROTOCOL_SET.has(inputs.protocol)) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: unknown protocol '${inputs.protocol}'.`,
    );
  }
  if (!Number.isFinite(inputs.amountUsd) || inputs.amountUsd < 0) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: amountUsd must be finite and non-negative.`,
    );
  }
  if (!USER_ID_RE.test(inputs.state.userId)) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: userId must match ${USER_ID_RE.source}.`,
    );
  }
  ...
  if (hfFloor <= 0n) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: policy.hfFloor must be > 0.`,
    );
  }
  if (hfBufferBps < 0n) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: policy.hfBufferBps must be >= 0.`,
    );
  }
  if (!Number.isFinite(thresholdUSD) || thresholdUSD < 0) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: policy.autoApprovalThresholdUSD must be finite and >= 0.`,
    );
  }
  if (!Number.isFinite(ttlMs) || ttlMs <= 0) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: policy.proposalTtlMs must be finite and > 0.`,
    );
  }
  ...
  try {
    ...
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/agent] runPropose: findPendingByAgent failed: ${safe.message}`,
      safe,
    );
  }
  if (existing !== null) {
  ...
  try {
    ...
    if (isPgUniqueViolation(err)) {
      ...
      try {
        ...
        throw new ConciergeError(
          'RpcError',
          `[@mpilot/agent] runPropose: post-unique-violation recovery read failed: ${safeRec.message}`,
          safeRec,
        );
      }
      if (winner !== null) {
      ...
      throw new ConciergeError(
        'InvariantViolation',
        `[@mpilot/agent] runPropose: unique_violation fired but no pending row found for agent ${inputs.state.agentId}.`,
      );
    }
    const safe = sanitizeError(err);
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/agent] runPropose: insert failed: ${safe.message}`,
      safe,
    );
  }
  ...
  if (!parsed.success) {
    throw new ConciergeError(
      'InvariantViolation',
      `[@mpilot/agent] runPropose: malformed event payload: ${parsed.error.message}`,
    );
  }
  ...
  try {
    ...
    throw new ConciergeError(
      'RpcError',
      `[@mpilot/agent] runPropose: publish failed (proposalId=${inserted.id}): ${safe.message}`,
      safe,
    );
  }
