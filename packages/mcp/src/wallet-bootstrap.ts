...
export function bootstrapWallet(opts: BootstrapOpts = {}): WalletConfig {
  ...
  if (!RPC_URL_RE.test(rpcUrl)) {
    throw new Error(
      `[@mpilot/mcp] wallet-bootstrap: CONCIERGE_RPC_URL has hostile shape (got '${rpcUrl.slice(0, 64)}'). Expected http(s) origin.`,
    );
  }
  ...
  try {
    ...
    safeUnlink(tmpPath);
    if (err instanceof Error && err.message.startsWith('[@mpilot/mcp]')) throw err;
...
function wrapWalletErr(err: unknown, op: string, path: string): Error {
  ...
  return new Error(`[@mpilot/mcp] wallet-bootstrap: ${op} at ${path} failed (${code}): ${msg}`);
}
...
function tryReadConfig(configPath: string): WalletConfig | null {
  ...
  if (parsed === null) {
    throw new Error(
      `[@mpilot/mcp] wallet-bootstrap: config at ${configPath} exists but is malformed. Refusing to overwrite. Move or delete it manually.`,
    );
  }
  return parsed;
}
