...
- **Default install path**: stdio bin — `npx -y @concierge/mcp`.
...
```bash
claude mcp add concierge -- npx -y @concierge/mcp
```
...
{
  "servers": {
    "concierge": { "command": "npx", "args": ["-y", "@concierge/mcp"] }
  }
}
```
...
```json
{
  "context_servers": {
    "concierge": {
      "command": { "path": "npx", "args": ["-y", "@concierge/mcp"] }
    }
  }
}
```
...
Cline settings → MCP Servers → add: command `npx`, args `-y @concierge/mcp`.
