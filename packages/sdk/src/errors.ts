...
export class ConciergeError extends Error {
  ...
  constructor(
    ...
    if (!isConciergeErrorType(type)) {
      throw new TypeError(
        `[@mpilot/sdk] ConciergeError: unknown type "${String(type)}" — expected one of: ${CONCIERGE_ERROR_TYPES.join(', ')}.`,
      );
    }
    this.type = type;
  ...
  toJSON(): Record<string, unknown> {
    ...
    if (this.metadata !== undefined) {
      try {
        ...
        console.error(
          `[@mpilot/sdk] ConciergeError.toJSON: failed to serialize metadata for type "${this.type}":`,
          serializationErr,
        );
