...
- **Stale adapter deps BANNED.** `@coinbase/agentkit-vercel-ai-sdk` (15mo stale), `@goat-sdk/*` (4-15mo stale), `@openai/agents` (15mo stale). Write ~30-LOC adapter from `@mpilot/tools` to the framework's core.
...
- **One source of truth for addresses: `@mpilot/shared/addresses.ts`.** Runtime reads from shared; tests assert against shared.
...
- `apps/` — `web/` (Next.js — dogfoods `@mpilot/react-ui`) + `mcp/` (Cloudflare Worker wrapper) + `worker/` (BullMQ tick worker, Fly.io).
