...
**How Concierge qualifies:** RealClaw skill packaging (NOT Byreal Skills CLI which is Solana-only, NOT Byreal Perps CLI which is Hyperliquid-only). Concierge ships as `npx skills add @concierge/mantle-agent` — a TypeScript skill installable into Claude Code / Claude Desktop / OpenClaw / RealClaw. Pattern verified via `byreal-git/byreal-agent-skills` itself (TS MIT, distributable via `npx skills add`) and `Magicianhax/mantle-active-trader` (Python RealClaw skill for Mantle).
...
> *"Concierge ships as a RealClaw-compatible skill installable via `npx skills add @concierge/mantle-agent` — packaging Concierge's autonomous DeFi agent into the official Byreal/RealClaw distribution channel. The skill exposes 7 Mantle-native action providers (Aave V3, Mantle DEX aggregation, Ethena sUSDe, Ondo USDY, mETH staking, Li.Fi bridging, ERC-8004 identity/reputation) callable from any RealClaw-compatible client. Every action the agent takes is signed via an ERC-4337 session key the user controls and recorded as a permanent ERC-8004 reputation attestation on Mantle."*
