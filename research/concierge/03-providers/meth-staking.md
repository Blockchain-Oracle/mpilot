# mETH Staking — mPilot Domain Knowledge

## What this is
mETH is Mantle's native ETH liquid staking token. ETH is staked via Mantle LSP's `Staking` contract on **Ethereum L1** → mETH is minted (non-rebasing, price grows). mETH is then bridged to Mantle Mainnet as the canonical ETH-yield asset of the chain. mPilot uses mETH as the **ETH-denominated yield asset** for users who hold ETH on Mantle and want native staking yield (~3.5% APR) without the L1 friction. Optional path: convert mETH → cmETH (restaked mETH via Karak / EigenLayer) for additional ~1-2% restaking yield.
...
### Mantle Sepolia (5003)
**[UNVERIFIED]**: a Mantle LSP testnet exists (METHL2 contract). mPilot spec should either mock or skip Sepolia for this provider.
...
mPilot does NOT mint/burn mETH on Mantle — it only holds it and trades it. Surface:
```solidity
...
### Gotchas
- `unstakeRequest` on L1 returns an NFT-style request — track the `requestID`. mPilot does not handle L1 directly (out of scope), but the docs spec mentions it for completeness.
- `mETHToETH` is a **price oracle**: 1 mETH ≈ 1.0929 ETH today (verified). This number grows every block from staking rewards. mPilot reads it via a cross-chain oracle (Redstone/Chainlink on Mantle) — **not direct L1 call**.
...
- Bridge in/out has LayerZero gas costs — mPilot avoids it for amounts < ~1 ETH.
...
### Non-rebasing model
mETH balance is constant; ETH-denominated value rises. Differs from Lido's stETH (rebases). mPilot HF calculations using mETH as collateral (if Aave ever lists it — currently not listed) would use the oracle's USD price (= mETH/ETH × ETH/USD).
...
- mPilot default: do **not** auto-restake. Require user opt-in via explicit "enable cmETH" flag.

### Why mETH at all for mPilot?
...
1. **DEX-pool depth**: WETH/mETH pool on Mantle is thin (~few hundred ETH). mPilot max single trade = $10K equivalent. Above that, the cross-chain L1-stake path is the only safe option.
2. **Exchange-rate oracle lag**: Mantle's mETH/ETH oracle may lag L1 by minutes-hours. mPilot treats spot DEX price vs oracle delta > 100bp as "abnormal" and pauses.
3. **Slashing**: Mantle LSP validators slashing event → mETH oracle drops → DEX panic. mPilot auto-divest trigger: mETH/ETH oracle drops >3% in 1h.
...
6. **Aave listing gap**: currently no Aave V3 Mantle market for mETH (only wrsETH). mPilot has no leverage loop here — it's hold-only. If Aave adds mETH (E-Mode "ETH correlated" category), mPilot can copy the sUSDe wedge pattern.
...
4. **mPilot wedge fit**: the locked wedge is stablecoin-denominated (sUSDe→USDC leverage). mETH is ETH-denominated and currently has no Aave market. Is mETH provider in MVP, or post-MVP? Recommend: ship as "hold-and-display" only in MVP. No leverage path until Aave lists.
