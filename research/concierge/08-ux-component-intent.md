...
- **Content:** Code block (`npm install @mpilot/sdk`), 3-line example showing an agent being spun up, "Read the docs" CTA.
...
#### `<Footer>`
- Primary nav back to all sections, GitHub link, X link, Docs link, MCP server install snippet (`npx skills add @mpilot/mantle-agent`), copyright + MIT license note.
...
- `/docs/providers` — `@mpilot/<provider>` per-provider reference (mirrors `03-providers/` content)
